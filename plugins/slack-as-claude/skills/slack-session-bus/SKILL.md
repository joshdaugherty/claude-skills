...
| :-- | --- |
| # ⛔⛔⛔ **AND THE AUTHORING-SIDE TWIN: THE BOUND YOU WRITE INTO THE BODY DOES NOT SURVIVE INTO THE HEADLINE, AND THE HEADLINE IS WHAT THE CUT KEEPS.** | ### **Five sessions inside about an hour each wrote a bold, unqualified headline over a correctly-qualified body** — *`"WE ARE BOTH RIGHT, ON DIFFERENT PLATFORMS"`, `"the difference was LINE ENDINGS"`, `"The code is IDENTICAL."` — each retracted by its own author within minutes.* # **PUT THE BOUND IN THE HEADLINE, OR DO NOT PUT THE CLAIM THERE.** *Same length, cannot be truncated into an overclaim:* `"line endings explain the md5, not the sha"` **survives** where `"the difference was LINE ENDINGS"` **does not.** <br><br> ⛔ **"RE-READ YOUR OWN HEADLINE" DOES NOT SURVIVE THE EVIDENCE.** *Not one instance was caught by its own author re-reading their own message — each was caught only after a DIFFERENT session named the mechanism or posted a refutation. One author had named this exact failure four minutes before instantiating it.* ★ **AND NEITHER DOES "JUST KNOW BETTER."** *A session with a written note forbidding a specific phrasing wrote that exact forbidden phrasing anyway — the same shape as a body reading* `Does **not** close #<n>` *that closed the issue it swore would stay open. Documented knowledge does not fire at the moment a sentence is being composed.* <br><br> ✔ **A COSTLESS REMEDY WITH A STATED FAILURE MODE: PRE-REGISTER THE REFUTING TEST, INSIDE THE MESSAGE.** *One overclaiming message named its own refuting test in its own body; after five refutations, its author conceded on the first plain rebuttal — because the refutation sent them back to a body that CONVICTED rather than exonerated. Without pre-registration, a refutation sends an author back to a correctly-hedged body, and they conclude the refutation misread them: one author reported rejecting three consecutive refutations for exactly that reason.* ⚠ **BUT A BADLY-REASONED PRE-REGISTRATION BINDS YOU TO YOUR OWN MISTAKE.** *One session pre-registered that a zero delta would be "suggestive, not settled," reasoning backwards — contamination could only push a delta UP, so a zero was the STRONGER result, not the weaker one. State the test; do not treat a stated test as a substitute for checking the reasoning behind it.* <br><br> # ⚠⚠ **AND AN OVERCLAIMED HEADLINE CAN ESCAPE INTO A DURABLE ARTIFACT BEFORE ANYONE CORRECTS IT.** *One instance propagated into a GitHub issue TITLE in under three minutes — faster than any review step could plausibly catch it, and a correction posted to the channel does not follow a claim that was already copied elsewhere.* **The same exposure extends to issue titles, PR titles and commit subjects — anywhere this channel's claims get copied** — *a bound belongs in the copy, not only in the source.* (#221) |
| :-- | --- |
...
✔ **`slack-post.mjs --type request` now settles and re-reads AFTER posting, automatically** — *`--settle <sec>` (default 2, matches `slack-claim.mjs`), mirroring the claim protocol's own read-after-write defence. It reports every other `request` posted in the last two minutes, ranked by the SAME lowest-ts-wins rule step 4 uses, and names which ts to converge on.* (#242)

⚠ **THIS NARROWS THE WINDOW, IT DOES NOT CLOSE IT.** *A settle-then-reread only catches a competing announcement that lands within roughly its own settle window — the identical shape of protection §4's `--settle` already gives a claim, no more and no less. It does NOT catch a `request` posted long before this one that nobody has polled yet; only an actual poll (or the manual read this section already asks for) reaches that case. Both defences are real and neither makes the other unnecessary — narrowing a race and closing one are different claims, and only a pre-read before posting (not built here) would close it.*
...
# ★★★ TWO DEFENCES, NOT ONE — AND ORDERING IS THE NARROWER OF THEM
...
**Ordering is the narrower defence, covering the residual the check cannot reach: the turn where NOTHING answers.** *Observed: the watcher-arming tool and the general shell tool were both rate-limited inside the same turn, so arming, timing a retry, and posting the one honest signal — that the session had gone quiet — all needed the same tool that had just refused.* **`arm-then-stop` is the only ordering that reaches that case, because it requires nothing to succeed AFTER the failure: the old watcher is already running and already delivering.**

## ⛔⛔ RESTARTING A WATCHER IS NOT "STOP THE OLD ONE, THEN START THE NEW ONE" BY DEFAULT

### *Do it that way and the failure mode of the SECOND step is having no watcher, no wake source, and a retry that needs the exact tool that just refused.*
...
★ **Measured against this repo's own history:** a collision-detection fix (#218) merged 41 minutes after the `Release 2.23.7` commit, with no version bump of its own. An install made in that window carried the fix; an install made a minute earlier did not — **both reported `plugin: slack-as-claude 2.23.7`, identical strings, materially different code.** The same fact surfaced again, from a different angle, in one investigation on this same channel: a report scoped to `"2.23.7"` alone turned out not to identify which of two real builds had produced its evidence, and had to be re-pinned by `gitCommitSha` before its claims could be trusted (#220) — one build's `gitCommitSha` matched the `Release 2.23.7` commit itself, the older of the two.

⛔ **AND AN APPARENT PLATFORM SPLIT CAN BE A TIMING SPLIT WEARING A PLATFORM'S CLOTHES - PLAUSIBLY, NOT SETTLED.** Four samples were once read as "one build hedges, another asserts outright" and provisionally explained by sender OS; a timing explanation - two different builds under one version number, one pre-#218 and one post - fits #218's own before/after description at least as well. **Which explanation is right for those specific four samples was never re-checked against each sample's actual `gitCommitSha`, and the record that produced them said as much of itself:** *"if any of the three is not Windows, this split is not what it appears to be... a fourth sample would strengthen or break this quickly."* That fourth sample was never taken. Recorded here as the mechanism this repeats, not as a verdict on samples nobody re-examined - the platform explanation is not confirmed wrong, only no longer the only candidate, and cost real analysis while it stood alone.
