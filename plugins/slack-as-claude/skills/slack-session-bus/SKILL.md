...
⛔⛔⛔ **AND ONE MORE THING, STATED AS UNVERIFIED RATHER THAN GUESSED: `verifyBotId()`'s reading of `msg.bot_id` has been confirmed against `auth.test` and Slack's `bot_message`-subtype docs, but NOT against a real `conversations.history` response for an ordinary bot post — a different endpoint, and a different shape is not ruled out (e.g. `msg.bot_profile?.id` instead of, or alongside, a top-level `msg.bot_id`).** *Shipped this way on explicit instruction rather than blocked on provisioning a second live token — this is exactly this repo's own tier-3 case: name the observer you lack, and say so, instead of guessing. Until someone runs that live check (post one message with a real coordinator token, read it back, inspect the raw object), treat every `!NOT-FROM-COORDINATOR` and `+coordinator-verified` this file has ever printed as UNCONFIRMED against real Slack behaviour, not merely untested by fixture.*
