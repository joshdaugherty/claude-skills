...
✔ **`verifyBotId()`'s reading of `msg.bot_id` is now measured, not just reasoned from `auth.test`.** *A real `conversations.history` read of an ordinary bot-posted message (this workspace, `#bus`, 2026-09-04) carries `bot_id` at the top level — `bot_profile.id` repeats it. `--raw` now prints both fields, specifically so this can be checked directly rather than inferred. This is a schema fact about how Slack tags any bot-authored message, so it does not depend on which particular token posted it — a second, distinct coordinator token was not needed to settle it.*
